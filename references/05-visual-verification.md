...
**为什么关卡 1 全绿仍不够**：基线截图与 clone 快照出自**同一次运行态定格**——scroll-reveal 元素在序列化时回退到 `opacity: 0`，原站截图和 clone 会"一样地错"，像素 diff 为 0；同理，JS 驱动的悬停/定时/滚动动效丢失后，静止态截图完全相同。**关卡 0~2 对这类缺陷全部失明**，必须用两份确定性清单兜底：

1. **运行态定格审计**：grep clone DOM 里的 `opacity: 0`、中间值内联 transform、空 canvas，逐条归类处置（命令与归类规则见 `06-animation.md` 前置节）。
2. **动效清单核销**：grep 已下载 bundle 里的动效原语（`useScroll`/`whileHover`/`setInterval`/`enterFrom` 等，完整对照表见 `06-animation.md` 第 1.5 节），每个命中登记处置结论。

两份清单都要求"每条命中有归类记录"，与关卡 0 一样是确定性检查：有遗漏就是有，没有就是没有。
...
全部关卡（0 / 1 / 1.5 / 2）均通过后，方可在 `report.html` 中标记「人工确认：已确认」，并将报告归档。
