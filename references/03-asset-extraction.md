...
>
> **文件名编码约定**（脚本已内置，排障时须知）：非 ASCII 文件名（中文、空格、括号）在 URL 里是百分号编码。`download-assets.mjs` 的落盘策略是**磁盘存解码后的原名、HTML/map 引用保持编码名**——静态服务器会把请求 pathname 解码后找文件，两者才对得上。若见到"引用正确却整批 404、磁盘文件名含字面 `%E4%B8%AD`"，就是这条被破坏了（旧产物可用 `urllib.parse.unquote` 批量重命名修复）。反向不可行：磁盘存编码名会 404，引用存解码名则文件名里的空格会破坏 srcset 语法。
