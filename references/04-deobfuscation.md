...
| **Tier 1** | 按需：Step 5 动效复刻中，CSS 提取/标准库复用不够时启用（映射见 `06-animation.md` 第七节） | 仅驱动视觉/动效的核心模块 | 中（分钟级） | 数个关键模块 |
...
## 四、Tier 1 — 智能深度还原（按需执行）

Tier 1 是**按需执行**的深度反混淆级别——在 Step 5 动效复刻中，当 CSS 提取与标准库黑盒复用（`06-animation.md` 第七节的映射表）不足以还原动效时才启用；不要在 Step 4 验收通过之前预先投入（分钟级成本，若 clone 本身还没修好会白做）。它**只处理驱动视觉效果和动效的核心模块**，其余业务逻辑（路由、数据请求、状态管理）保持黑盒不处理。
