...
└──────┬──────┘
       ...
       ▼
┌─────────────┐
│  Step 5     │  浏览器 MCP + bundle 扫描 + 手工
│  动效复刻   │  ─────────────────────────────────────────────────────
│             │  ① 先跑运行态定格审计 + bundle 动效原语清单
│             │    （06 前置节 + 1.5 节，离线可用，核销依据）
│             │  ② CSS transition / animation / GSAP / Lottie 等逐一
│             │    实现到 clone；用浏览器 MCP 录制/回放验证视觉一致
│             │  ③ 清单全部核销（已复刻/静态可接受/放弃+理由）
└──────┬──────┘
       │
       ▼
┌─────────────┐
...
**前置硬门控（先于三重对比）：资源完整性体检。** grep clone 产物中残留的外链 / 根绝对路径引用，必须为 0，再进入像素/结构/样式对比——像素分数会被懒加载状态、页面高度差异稀释，单看分数发现不了背景图/图标 404。命令与判定见 `05-visual-verification.md` 关卡 0。
