...
全部关卡（资源体检 + 自动指标 + 定格审计/动效核销 + 人工确认）均通过，方可归档。资源体检为何必须独立：像素/结构/样式分数会被懒加载状态、页面高度差异稀释，单看分数发现不了资源缺失，必须 grep 残留引用确定性兜住（详见 `05` 关卡 0）。

**关卡 1.5（定格审计 + 动效清单核销）为何必须独立**：基线截图与 clone 出自同一次运行态定格，scroll-reveal 回退到 `opacity:0`、JS 动效丢失时**两边"一样地错"，像素 diff 为 0**，分数类关卡全部失明。必须 ① grep clone DOM 的隐藏态/中间值定格逐条归类，② grep 已下载 bundle 的动效原语（`useScroll`/`whileHover`/`setInterval`/`enterFrom` 等）建清单逐条核销（详见 `06` 前置节与第 1.5 节、`05` 关卡 1.5）。bundle 扫描离线可用——原站不可达时这是唯一可靠的动效来源。
